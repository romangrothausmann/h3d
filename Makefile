...
ITKEXE+= file-series_writer_SDI watershed_morph_nX+border-thr watershed_morph_nX_marker_float fill_holes erode_bin_morph open_bin_morph label_overlap
...
%slices.done %ana.done %luBounds.done %VR.done %SRV.done %VE.done : %segmentation/autoSeg.done # this way rule will only be executed once for all listed *.done
	...
	find processing/ -size +10G -name "*.mha"  | egrep -v 'h3d_seg_Bt\+Arz-fm.mha|h3d_seg_B-fm.mha' | xargs pigz -v -f # just to save disc space, subsequent make-calls will unpack needed files (pigz used because of 4GB bug in ITK/VTK's MetaIO: https://issues.itk.org/jira/browse/ITK-3321)
